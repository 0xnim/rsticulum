...
struct PythonResponse {
    ...
    #[serde(default)]
    transport_type: Option<u64>,
    #[serde(default)]
    context_flag: Option<u64>,
    #[serde(default)]
    transport_id: Option<String>,
    #[serde(default)]
    announce_flags: Option<String>,
    #[serde(default)]
    error: Option<String>,
    #[serde(default)]
    derived_hex: Option<String>,
    #[serde(default)]
    token_hex: Option<String>,
    #[serde(default)]
    token_len: Option<u64>,
    #[serde(default)]
...
#[test]
fn python_parses_all_rust_packet_types() {
    let (mut s, mut r, mut c) = spawn();
    let d = addr(42);
    let dh = hex::encode(d.as_bytes());
    for (lbl, pkt, et, eh, ecf, etr) in [
        (
            "DATA",
            Packet::new_data(d, b"dp".to_vec()),
            0x00,
            64,
            0x00,
            0x00,
        ...
        (
            "ANNOUNCE",
            Packet::new_announce(d, b"ap".to_vec()),
            0x01,
            1,
            0x01,
            0x00,
        ...
        (
            "LINK_REQUEST",
            Packet::new_link_request(d, b"lr".to_vec()),
            0x02,
            64,
            0x00,
            0x00,
        ...
        (
            "PROOF",
            Packet::new_proof(d, 0xFF, b"pp".to_vec()),
            0x03,
            64,
            0x01,
            0x01,
        ...
        let resp = read(&mut r);
        assert_eq!(resp.status, "ok", "{lbl}: {resp:?}");
        assert_eq!(resp.packet_type.unwrap() as u8, et);
        assert_eq!(resp.hops.unwrap() as u8, eh);
...
fn announce_flags() {
    let (mut s, mut r, mut c) = spawn();
    send(&mut s, "ANNOUNCE_FLAGS");
    let resp = read(&mut r);
    let f = u8::from_str_radix(
        resp.announce_flags
            .as_deref()
            .unwrap()
    ...
    assert_eq!(f, 0x21);
    assert_eq!(
        rsticulum_packet::pack_flags(0x00, 0x01, 0x00, 0x00, 0x01),
        0x21
    );
    send(&mut s, "QUIT");
    c.wait().unwrap();
...
fn large_payload() {
    ...
    send(
        ...
        &format!(
            "PARSE {}",
            hex::encode(Packet::new_data(addr(55), p.clone()).to_bytes())
        ),
    );
    let resp = read(&mut r);
...
fn hkdf_matches_python() {
    ...
    assert_eq!(
        resp.derived_hex.as_deref().unwrap(),
        hex::encode(&rust_key),
...
#[test]
fn token_encrypt_decrypt_cross_compat() {
    let (mut s, mut r, mut c) = spawn();
    ...
    send(
        &mut s,
        &format!("TOKEN_DECRYPT {} {key_hex}", hex::encode(&token)),
    );
    let resp = read(&mut r);
    assert_eq!(resp.status, "ok", "python decrypt: {resp:?}");
    assert_eq!(
        resp.plaintext_hex.as_deref().unwrap(),
        pt_hex,
